...
Variables that must be fetched from the secret vault. Declaring them here tells the provisioner to retrieve the value at runtime rather than ask the user for it in plain text. Secrets are never written to `freeloader.yml`.

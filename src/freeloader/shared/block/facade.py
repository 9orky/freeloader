...
class Blocks:
    def __init__(self, blocks_root: Path | None = None) -> None:
        folder = blocks_root or Path(getenv("FREELOADER_BLOCKS", ""))
        self._repository = BlockRepository.load(folder)
    
    def get_manifest_configs(self, provider_names: list[str], full: bool = False) -> dict[str, dict[str, str]]:
        ...
        for provider in providers:
            for block in provider.blocks:
                configs[block.id] = block.dump_config(full=full)
        return configs

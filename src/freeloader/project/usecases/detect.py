...
def build_test_projects(cwd: Path) -> dict:
    if cwd != Path.cwd():
    ...
    for folder, commands in folders_to_commands.items():
        ...
        for command_name, command_str in commands.items():
            if command_name in ["init", "add"]:
                try:
                    subprocess.run(command_str, cwd=folder_path,
                                   shell=True, check=True)
                    project_paths.append(folder_path)
                except subprocess.CalledProcessError as e:
                    print(
                        f"Command '{command_str}' failed in folder '{folder_path}': {e}")
    ...
    for project_folder in project_paths:
        ...
        folders_to_stack[str(project_folder)
                         ] = stack_line or "No tech stack detected"

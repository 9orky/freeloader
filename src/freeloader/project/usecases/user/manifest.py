...
class ProjectManifest:
    ...
    @classmethod
    def create(cls, name: str, folder: Path, stack: dict[str, str], block_configs: dict[str, dict[str, ConfigValue]]) -> "ProjectManifest":
        assert not cls.exists(
            folder), f"Project manifest already exists in {folder}"
    ...
    def delete(self) -> None:
        assert self.manifest_file.is_file(
        ), f"Project manifest {self.manifest_file} does not exist"
    ...
    @staticmethod
    def _create_stack(stack: dict[str, str]) -> ManifestTechStack:
